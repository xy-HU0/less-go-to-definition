...
export class LessDefinitionProvider implements vscode.DefinitionProvider {
  ...
  private getVariableAtPosition(document: vscode.TextDocument, position: vscode.Position): RegExp | undefined {
    ...
    // A variable usage is a variable that appears AFTER a colon on the same line.
    // Example: "color: @my-color;" or "@h1-font-size: @base-font-size;"
    const firstColonIndex = lineText.indexOf(':');
    if (firstColonIndex !== -1) {
        // This regex finds both @variable and @{variable}
        const variableRegex = /@\{?([\w-]+)\}?/g;
        let match;
        while ((match = variableRegex.exec(lineText)) !== null) {
            // Check if the variable appears after the first colon
            if (match.index > firstColonIndex) {
  ...
  private async resolveImportPath(importPath: string, currentFileUri: vscode.Uri): Promise<vscode.Uri | undefined> {
    ...
    for (const includePath of includePaths) {
      const potentialUri = vscode.Uri.file(path.resolve(includePath, importPath));
      try {
    ...
    try {
      ...
      for (const line of lines) {
        ...
        if (importMatch && importMatch[1]) {
          ...
          if (importedFileUri) {
            const importedFileContentBytes = await vscode.workspace.fs.readFile(importedFileUri);
            const importedFileContent = Buffer.from(importedFileContentBytes).toString('utf8');

            const locationInImportedFile = this.findDefinitionInFile(importedFileContent, definitionRegex, importedFileUri);
            if (locationInImportedFile) {
              return locationInImportedFile;
            }
          }
